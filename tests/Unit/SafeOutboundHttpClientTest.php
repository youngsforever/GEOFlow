...
use App\Support\GeoFlow\OutboundHttpSecurity;
use Closure;
...
class SafeOutboundHttpClientTest extends TestCase
{
    ...
    #[Test]
    public function the_global_boundary_security_options_override_malicious_caller_options(): void
    {
        config(['geoflow.outbound_ai_max_bytes' => 1024]);
        $client = $this->client(['public.test' => ['93.184.216.34']], new RecordingOutboundTransport);
        $callerHeadersCalled = false;
        $middleware = OutboundHttpSecurity::middleware(static fn (): SafeOutboundHttpClient => $client);
        $handler = $middleware(static fn ($request, array $options): array => $options);

        $options = $handler(new PsrRequest('GET', 'https://public.test/v1/chat/completions'), [
            'allow_redirects' => true,
            'proxy' => 'http://evil-proxy.test:8080',
            'on_headers' => static function () use (&$callerHeadersCalled): void {
                $callerHeadersCalled = true;
            },
            'curl' => [
        ...
        $this->assertSame('https://public.test/v1/chat/completions', $options['curl'][CURLOPT_URL]);
        $this->assertSame(443, $options['curl'][CURLOPT_PORT]);
        $this->assertArrayNotHasKey(CURLOPT_CONNECT_TO, $options['curl']);
        $this->assertSame(['public.test:443:93.184.216.34'], $options['curl'][CURLOPT_RESOLVE]);
        ...
        $options['on_headers'](new PsrResponse(200, ['Content-Length' => '12']));
        $this->assertTrue($callerHeadersCalled);

        $this->expectExceptionObject(new OutboundRequestBlockedException('response_too_large'));
        $options['on_headers'](new PsrResponse(200, ['Content-Length' => '2048']));
    }

    /**
     * @return array<string, array{string, list<string>, string, string, string}>
     */
    public static function normalizedGlobalBoundaryTargets(): array
    {
        return [
            'default port' => [
                'https://PUBLIC.test./v1/models?mode=fast',
                ['93.184.216.34'],
                'https://public.test/v1/models?mode=fast',
                'public.test',
                'public.test:443:93.184.216.34',
            ],
            'explicit default port' => [
                'https://PUBLIC.test.:443/v1/models?mode=fast',
                ['93.184.216.34'],
                'https://public.test/v1/models?mode=fast',
                'public.test',
                'public.test:443:93.184.216.34',
            ...
            'non default port' => [
                'https://PUBLIC.test.:8443/v1/models?mode=fast',
                ['93.184.216.34'],
                'https://public.test:8443/v1/models?mode=fast',
                'public.test:8443',
                'public.test:8443:93.184.216.34',
            ],
            'ipv6 resolution' => [
                'https://PUBLIC.test./v1/models?mode=fast',
                ['2606:4700:4700::1111'],
                'https://public.test/v1/models?mode=fast',
                'public.test',
                'public.test:443:[2606:4700:4700::1111]',
            ],
        ];
    }

    #[Test]
    #[DataProvider('normalizedGlobalBoundaryTargets')]
    public function the_global_boundary_normalizes_the_handler_uri_and_host_before_using_the_dns_pin(
        string $inputUrl,
        array $addresses,
        string $expectedUrl,
        string $expectedHost,
        string $expectedResolve,
    ): void {
        $client = $this->client(['public.test' => $addresses], new RecordingOutboundTransport);
        $middleware = OutboundHttpSecurity::middleware(static fn (): SafeOutboundHttpClient => $client);
        $handler = $middleware(static fn ($request, array $options): array => [
            'request' => $request,
            'options' => $options,
        ]);

        $result = $handler(
            new PsrRequest('GET', $inputUrl, ['Host' => 'attacker.internal']),
            [
                'curl' => [
                    CURLOPT_RESOLVE => ['attacker.invalid:443:127.0.0.1'],
                ],
            ],
        );

        $this->assertSame($expectedUrl, (string) $result['request']->getUri());
        $this->assertSame([$expectedHost], $result['request']->getHeader('Host'));
        $this->assertSame([$expectedResolve], $result['options']['curl'][CURLOPT_RESOLVE]);
    }

    #[Test]
    public function the_global_boundary_removes_abstract_unix_socket_overrides_when_supported(): void
    {
        if (! defined('CURLOPT_ABSTRACT_UNIX_SOCKET')) {
            $this->markTestSkipped('This cURL build does not expose abstract Unix socket support.');
        }

        $abstractSocketOption = constant('CURLOPT_ABSTRACT_UNIX_SOCKET');
        $client = $this->client(['public.test' => ['93.184.216.34']], new RecordingOutboundTransport);
        $middleware = OutboundHttpSecurity::middleware(static fn (): SafeOutboundHttpClient => $client);
        $handler = $middleware(static fn ($request, array $options): array => $options);

        $options = $handler(new PsrRequest('GET', 'https://public.test/v1/models'), [
            'curl' => [
                $abstractSocketOption => 'geoflow-internal.sock',
            ],
        ]);

        $this->assertArrayNotHasKey($abstractSocketOption, $options['curl']);
    }

    /**
     * @return array<string, array{string, string, string, int, string}>
     */
    public static function publicIpLiteralGlobalBoundaryTargets(): array
    {
        return [
            'ipv4 default port' => [
                'https://93.184.216.34/v1/models?mode=fast',
                'https://93.184.216.34/v1/models?mode=fast',
                '93.184.216.34',
                443,
                'v4',
            ...
            'ipv6 default port' => [
                'https://[2606:4700:4700::1111]/v1/models?mode=fast',
                'https://[2606:4700:4700::1111]/v1/models?mode=fast',
                '[2606:4700:4700::1111]',
                443,
                'v6',
            ...
            'ipv6 non default port' => [
                'https://[2606:4700:4700::1111]:8443/v1/models?mode=fast',
                'https://[2606:4700:4700::1111]:8443/v1/models?mode=fast',
    ...
    #[Test]
    #[DataProvider('publicIpLiteralGlobalBoundaryTargets')]
    public function the_global_boundary_keeps_public_ip_literal_requests_fixed_without_resolve_entries(
        string $inputUrl,
        string $expectedUrl,
        string $expectedHost,
        int $expectedPort,
        string $expectedIpVersion,
    ): void {
        $client = $this->client([], new RecordingOutboundTransport);
        $middleware = OutboundHttpSecurity::middleware(static fn (): SafeOutboundHttpClient => $client);
        $handler = $middleware(static fn ($request, array $options): array => [
        ...
        $this->assertSame($expectedUrl, (string) $result['request']->getUri());
        $this->assertSame([$expectedHost], $result['request']->getHeader('Host'));
        $this->assertSame($expectedUrl, $result['options']['curl'][CURLOPT_URL]);
        $this->assertSame($expectedPort, $result['options']['curl'][CURLOPT_PORT]);
        $this->assertSame($expectedIpVersion, $result['options']['force_ip_resolve']);
        $this->assertArrayNotHasKey(CURLOPT_RESOLVE, $result['options']['curl']);
    }

    #[Test]
    public function the_global_boundary_forces_identity_encoding_and_rejects_encoded_responses(): void
    {
        $client = $this->client(['public.test' => ['93.184.216.34']], new RecordingOutboundTransport);
        $captured = [];
        $middleware = OutboundHttpSecurity::middleware(static fn (): SafeOutboundHttpClient => $client);
        $handler = $middleware(static function ($request, array $options) use (&$captured): PsrResponse {
            $captured = ['request' => $request, 'options' => $options];

            return new PsrResponse(200, [], 'ok');
        });

        $response = $handler(
            new PsrRequest('GET', 'https://public.test/v1/models', ['Accept-Encoding' => 'gzip']),
            [
                'decode_content' => true,
                'headers' => [
                    'accept-encoding' => 'br',
        ...
        $this->assertSame(['identity'], $captured['request']->getHeader('Accept-Encoding'));
        $this->assertFalse($captured['options']['decode_content']);
        $this->assertSame(['Accept-Encoding' => 'identity'], $captured['options']['headers']);
        $this->assertSame('identity', $captured['options']['curl'][CURLOPT_ENCODING]);
        $this->assertArrayNotHasKey(CURLOPT_HTTPHEADER, $captured['options']['curl']);

        $this->expectExceptionObject(new OutboundRequestBlockedException('encoded_response_forbidden'));
        $captured['options']['on_headers'](new PsrResponse(200, ['Content-Encoding' => 'gzip']));
    }

    #[Test]
    public function the_global_boundary_post_checks_direct_response_bodies(): void
    {
        config(['geoflow.outbound_ai_max_bytes' => 1024]);
        $client = $this->client(['public.test' => ['93.184.216.34']], new RecordingOutboundTransport);
        $middleware = OutboundHttpSecurity::middleware(static fn (): SafeOutboundHttpClient => $client);
        $handler = $middleware(static fn (): PsrResponse => new PsrResponse(200, [], str_repeat('x', 1025)));

        $this->expectExceptionObject(new OutboundRequestBlockedException('response_too_large'));
        $handler(new PsrRequest('GET', 'https://public.test/v1/models'), []);
    }

    #[Test]
    public function the_global_boundary_post_checks_promise_response_bodies(): void
    {
        config(['geoflow.outbound_ai_max_bytes' => 1024]);
        $client = $this->client(['public.test' => ['93.184.216.34']], new RecordingOutboundTransport);
        $middleware = OutboundHttpSecurity::middleware(static fn (): SafeOutboundHttpClient => $client);
        $handler = $middleware(
            static fn (): PromiseInterface => new FulfilledPromise(new PsrResponse(200, [], str_repeat('x', 1025)))
        );

        $promise = $handler(new PsrRequest('GET', 'https://public.test/v1/models'), []);
    ...
    #[Test]
    public function the_global_boundary_limits_unknown_size_streaming_response_bodies_as_they_are_read(): void
    {
        config(['geoflow.outbound_ai_max_bytes' => 1024]);
        $client = $this->client(['public.test' => ['93.184.216.34']], new RecordingOutboundTransport);
        $middleware = OutboundHttpSecurity::middleware(static fn (): SafeOutboundHttpClient => $client);
        ...
        $handler = $middleware(
            static fn (): PromiseInterface => new FulfilledPromise(new PsrResponse(200, [], $stream))
        );

        $promise = $handler(new PsrRequest('GET', 'https://public.test/v1/models'), ['stream' => true]);
        $response = $promise->wait();
        $this->assertSame(str_repeat('x', 1024), $response->getBody()->read(1024));
    ...
    #[Test]
    public function streaming_intent_uses_the_pinned_handler_and_returns_a_size_limited_stream(): void
    {
        config(['geoflow.outbound_ai_max_bytes' => 1024]);
        $client = $this->client(['public.test' => ['93.184.216.34']], new RecordingOutboundTransport);
        $middleware = OutboundHttpSecurity::middleware(static fn (): SafeOutboundHttpClient => $client);
        ...
        $handlerOptions = [];
        $response = static function (): PsrResponse {
            $remaining = 1025;
            $stream = new PumpStream(static function (int $requested) use (&$remaining): string|false {
                if ($remaining === 0) {
                    return false;
            ...
            return new PsrResponse(200, [], $stream);
        };
        $pinnedHandler = static function ($request, array $options) use (&$selectedHandler, &$handlerOptions, $response): PromiseInterface {
            $selectedHandler = 'pinned';
            $handlerOptions = $options;

            return new FulfilledPromise($response());
        };
        $streamHandler = static function () use (&$selectedHandler, $response): PromiseInterface {
            $selectedHandler = 'stream';

            return new FulfilledPromise($response());
        };
        $handler = $middleware(Proxy::wrapStreaming($pinnedHandler, $streamHandler));

        $promise = $handler(new PsrRequest('GET', 'https://public.test/v1/models'), ['stream' => true]);
        $guardedResponse = $promise->wait();

        $this->assertSame('pinned', $selectedHandler);
        $this->assertFalse($handlerOptions['stream']);
        $this->assertSame(['public.test:443:93.184.216.34'], $handlerOptions['curl'][CURLOPT_RESOLVE]);
        $this->assertInstanceOf(ResponseSizeLimitedStream::class, $guardedResponse->getBody());
        $this->assertSame(str_repeat('x', 1024), $guardedResponse->getBody()->read(1024));

        $this->expectExceptionObject(new OutboundRequestBlockedException('response_too_large'));
        $guardedResponse->getBody()->read(1);
    }

    #[Test]
    public function the_global_boundary_fails_closed_without_a_pinnable_curl_transport(): void
    {
        $client = $this->client(['public.test' => ['93.184.216.34']], new RecordingOutboundTransport);
        $handlerCalled = false;
        $middleware = OutboundHttpSecurity::middleware(
            static fn (): SafeOutboundHttpClient => $client,
            static fn (bool $requiresDnsPin): bool => false,
        );
        $handler = $middleware(static function () use (&$handlerCalled): PsrResponse {
            $handlerCalled = true;
        ...
        try {
            $handler(new PsrRequest('GET', 'https://public.test/v1/models'), ['stream' => true]);
            $this->fail('Expected the request to fail closed.');
        } catch (OutboundRequestBlockedException $exception) {
            $this->assertSame('pinning_unavailable', $exception->reasonCode);
        } finally {
    ...
    #[Test]
    public function the_global_boundary_rejects_encoded_mock_responses_even_when_on_headers_is_skipped(): void
    {
        $client = $this->client(['public.test' => ['93.184.216.34']], new RecordingOutboundTransport);
        $middleware = OutboundHttpSecurity::middleware(static fn (): SafeOutboundHttpClient => $client);
        $handler = $middleware(
            static fn (): PsrResponse => new PsrResponse(200, ['Content-Encoding' => 'br'], 'compressed')
        );
    ...
    #[Test]
    public function the_global_boundary_preserves_an_encoding_policy_failure_through_a_rejected_promise(): void
    {
        $client = $this->client(['public.test' => ['93.184.216.34']], new RecordingOutboundTransport);
        $middleware = OutboundHttpSecurity::middleware(static fn (): SafeOutboundHttpClient => $client);
        $handler = $middleware(static function ($request, array $options): PromiseInterface {
            try {
                $options['on_headers'](new PsrResponse(200, ['Content-Encoding' => 'gzip']));
            } catch (OutboundRequestBlockedException) {
                return new RejectedPromise(new \RuntimeException('HTTP handler wrapped the policy failure.'));
            }
    ...
    #[Test]
    public function global_boundary_runtime_failures_are_centrally_redacted(): void
    {
        $client = $this->client(['secret.test' => ['93.184.216.34']], new RecordingOutboundTransport);
        $middleware = OutboundHttpSecurity::middleware(static fn (): SafeOutboundHttpClient => $client);
        $handler = $middleware(static function (): never {
            throw new \RuntimeException('TLS failed for https://secret.test via 10.0.0.9 token=super-secret');
        });

        try {
            $handler(new PsrRequest('GET', 'https://secret.test/v1/models'), []);
            $this->fail('Expected transport failure.');
        } catch (OutboundRequestFailedException $exception) {
            $this->assertSame('outbound_request_failed', $exception->reasonCode);
            $this->assertSame('Outbound request failed.', $exception->getMessage());
